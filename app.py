...
if st.session_state.current_view == '대시보드':
    ...
    with col1:
        ...
        interests = st.text_area("관심 분야", value=st.session_state.user_profile['interests'], 
                                 placeholder="예: AI, 데이터 분석, 비즈니스")
    
    with col2:
        aptitude = st.text_area("적성 및 진로 목표", value=st.session_state.user_profile['aptitude'],
                               placeholder="예: 데이터 사이언티스트, 창업")
    
    if st.button("🎯 AI 추천 받기", type="primary"):
        if not API_KEY:
            st.error("⚠️ GEMINI_API_KEY 환경 변수를 설정해주세요.")
        elif not interests or not aptitude:
